...
- **팔레트가 한 글자 때문에 포기하지 않는다.** 파일명 검색이 `blogwrite` 로 **7회** 0건을 냈는데
  찾던 노트(`blog-writing-skill-…`)는 vault 에 있었다. 순수 subsequence 매칭이라 `write` 의 `e`
  하나가 대상에 없으면 `null` 을 낸다 — `write` 는 `writing` 의 접두사가 아니고 같은 것은 `writ`
  까지다. 두 가지를 넣되 **적용 지점을 나눴다**: ⑴ **구분자 제거**는 항상 본다(사람은
  `blogwrite` 라고 치지 `blog-write` 라고 치지 않는다). 하이픈이 끊어 놓은 연속 매칭이 살아나
  순위가 오른다 — 실측 144 → 784. ⑵ **굴절 접기**는 **0건일 때만** 연다. ⚠️ 질의와 대상을 **양쪽
  다** 같은 규칙으로 접는다 — 대상만 접으면 `parse` 가 `parsing` 을 못 찾는다. 🔴 어간으로 찾았으면
  **화면이 그렇다고 말한다** — 한글 IME 되돌리기와 같은 계약이다. 조용히 다른 것을 주면 사용자는
  왜 그게 나왔는지 모르고, 그러면 다음부터 결과 자체를 안 믿는다. ⚠️ **되는 질의는 절대 안 건드린다.**
  ⚠️ 편집거리가 아니라 **선언된 접미사 표**다(한글은 안 건드린다 — 조사·어미는 사전이 필요하고 이미
  값이 안 나온다고 잰 적이 있다). ⚠️ 정직하게 — 실측 7회 중 **3회**가 풀린다. `blogwritei` 같은
  타이핑 중간 상태는 편집거리가 아니면 못 잡는다.

### 수정

- **`lapis open` 이 연 노트를 스스로 덮어썼다.** 실사용 로그에 `[cli-open] 기동 열기 실패` 가 **5회**
  쌓여 있었고 이유가 그대로 적혀 있었다 — `Command plugin:window|set_focus not allowed by ACL`.
  창 권한 목록에 `set-focus` 가 없었다. 🔴 사라지는 경로가 고약하다: 노트를 여는 것은 **성공한 뒤**
  포커스에서 터지고, 그 예외를 잡은 쪽이 "빈 창을 남기지 않겠다"며 **지난 vault 를 복원**한다.
  그래서 방금 연 노트가 덮인다. 같은 자리의 `show` · `unminimize` 도 선언이 없었는데 이쪽은 아예
  삼켜져서 **최소화된 창이 안 돌아오고 로그에도 안 남았다.** 셋 다 선언했고, 포커스 실패는 이제
  이미 연 노트를 무르지 않는다. 🔴 목록을 손으로 유지하지 않는다 — 창 API 를 **부르는 곳**과 권한
  **선언**을 대조하는 검사가 생겼고, 권한을 하나 지워 우는 것까지 확인했다. ⚠️ Tauri 문서의 커스텀
  타이틀바 예시가 정확히 이 저장소가 갖고 있던 넷이다. **예시를 따라가면 예시가 안 쓰는 것이 빠진다.**
- **설치 안내가 만들지 않는 인스톨러를 가리켰다.** 앞 항목에서 MSI 를 기본 번들에서 뺐는데 README
  양쪽이 그대로 `bundle/msi/` 를 안내하고 있었다. 필요할 때 만드는 방법을 대신 적었다.
